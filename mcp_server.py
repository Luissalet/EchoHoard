"""Stdio MCP bridge for Echo's Hoard.
...
app itself (`python -m echo`, detached, on the port of ECHO_URL) and waits
for it, so a workspace that starts before the app still gets its tools;
ECHO_BRIDGE_AUTOSTART=0 turns that off.
"""
...
BASE_URL = os.environ.get("ECHO_URL", "http://127.0.0.1:5188").rstrip("/")
TOKEN_FILE = Path(
    os.environ.get("ECHO_TOKEN_FILE")
    or Path(os.environ.get("ECHO_DATA_DIR") or ROOT / "data") / "mcp-token"
)
NOT_RUNNING = "Open Echo's Hoard (python -m echo) so the assistant can reach the clipboard history."


def _healthy() -> bool:
    try:
        response = httpx.get(f"{BASE_URL}/api/health", timeout=4, trust_env=False)
        return response.status_code == 200 and response.json().get("service") == "echo-hoard"
    except Exception:
        return False
...
def ensure_running(timeout_s: float = 45.0) -> bool:
    ...
    if _healthy():
        return True
    if os.environ.get("ECHO_BRIDGE_AUTOSTART", "1") == "0":
        return False
    port = urlparse(BASE_URL).port or 5188
    env = {**os.environ, "ECHO_PORT": str(port), "PORT_STRICT": "1", "PYTHONUNBUFFERED": "1"}
    ...
    logs = Path(os.environ.get("ECHO_DATA_DIR") or ROOT / "data") / "logs"
    logs.mkdir(parents=True, exist_ok=True)
    with open(logs / "echo-app.log", "ab") as out:
        subprocess.Popen([sys.executable, "-m", "echo"], cwd=ROOT, env=env, stdin=subprocess.DEVNULL,
...
def _token() -> str:
    env = os.environ.get("ECHO_TOKEN")
    if env:
        return env.strip()
    return TOKEN_FILE.read_text(encoding="utf-8").strip()


class EchoBridge(FastMCP):
    ...
    def __init__(self, catalog: list[dict], instructions: str):
        super().__init__(name="echo-hoard", instructions=instructions)
...
def main() -> None:
    ...
    EchoBridge(catalog, instructions).run(transport="stdio")
