...
from echo.guard import check_request, host_of, install_guard, is_allowed_host, parse_allowed_hosts
...
def test_host_of_strips_scheme_path_port_and_case():
    assert host_of("LocalHost:5188") == "localhost"
    assert host_of("https://My-PC.ts.net:8443/x") == "my-pc.ts.net"
    assert host_of("[::1]:5188") == "[::1]"
...
def test_check_request_fetch_metadata_rules():
    local = {"host": "localhost:5188"}
    assert check_request("GET", local) is None  # curl / MCP bridge: no Sec-Fetch headers
    assert check_request("POST", {"host": "127.0.0.1:5188"}) is None
...
def test_check_request_origin_by_host_not_exact_string():
    ...
    assert check_request("GET", headers("http://localhost:5188"), allowed) is None
...
def test_app_without_allowed_hosts_is_local_only(client):
    ...
    assert client.get("/api/health", headers={"host": "[::1]:5188"}).status_code == 200
